...
diarrheaMeasurement.limited = 
    diarrheaMeasurement %>%
    select(dataid, childid, clusterid, block, svy, sex, agedays, diar7d)

diarrheaMeasurement.limited %>% head
```
Next up, let's simplify our adherence dataset. We are also missing a ton of data for rlnsp/lnsp (nearly 90% of rows), which is something to consider later.  
```{r}
...
mergedData = compoundTreatment %>% inner_join(diarrheaMeasurement.limited) %>% 
inner_join(compoundAdherence.limited) %>% inner_join(compoundCharacteristics.limited, by=c("block", "clusterid", "dataid"))
